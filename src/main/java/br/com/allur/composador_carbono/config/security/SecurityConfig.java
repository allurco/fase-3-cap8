    ...
    public SecurityFilterChain filtrarCadeiaDeSeguranca(HttpSecurity httpSecurity) throws Exception {
        ...
        return httpSecurity.csrf(csrf -> csrf.disable())
                ...
                .authorizeHttpRequests(authorize -> authorize
                        ...
                        .requestMatchers(HttpMethod.GET, "/api/alertas", "/api/alertas/**").hasRole("ADMIN")
                        .requestMatchers(HttpMethod.POST, "/api/alertas").hasRole("ADMIN")
                        .requestMatchers(HttpMethod.GET, "/api/compensacoes", "/api/compensacoes/**").hasRole("ADMIN")
                        .requestMatchers(HttpMethod.POST, "/api/compensacoes").hasRole("ADMIN")
                        .anyRequest()
                        .authenticated())
                .addFilterBefore(
                        verificarToken,
